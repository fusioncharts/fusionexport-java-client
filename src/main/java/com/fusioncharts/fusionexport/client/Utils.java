...
public class Utils {
	...
	public static HashMap<String, ByteArrayOutputStream> unzipToStream(byte[] response) throws ExportException {
		HashMap<String, ByteArrayOutputStream> files = new HashMap<String, ByteArrayOutputStream>(); 

		ByteArrayInputStream zipFileStream = new ByteArrayInputStream(response);
		...
		try {
			...
			while(ze != null){
				...
				/*
				while (zis.read(buffer, 0, BUFFER_SIZE) != -1) {
					bos.write(buffer);
				}
				*/
